...
const rediologyServicesData = {
  twoD_echo : {
    ...
    content: "2-Dimentional Echocardiography or ECHO is used to visualize the actual motion of the heart. On the monitor, a cone-shaped 2D ECHO view shows the real-time motion of the heart's structures."
  },
  CBCT: {
    id : "rdo_serv2",
  ...
  Xray : {
    ...
    content: "X-ray is a type of imaging technology that employs low ionizing radiation to capture the image of the body's bones and soft tissues.   "
  },
...
const RadiologySrvcCard = (props) => {
  return(
    <div className="r_imgbox_wrapper">
        <div className="r_imgbox_img" style={{
        ...
        <div className="r_imgbox_content p-3">
          <h2 className="r_heading_title_secondary r_imgbox_title mb-2"> {props.servInfo.name} </h2>
          <p className="r_imgbox_desc">
            {props.servInfo.shortDesc.substring(0,65)}...
          </p>
          <button type="button" className="btn btn-primary btn-sm">Know More</button>
        </div>
    </div>
  )
}
